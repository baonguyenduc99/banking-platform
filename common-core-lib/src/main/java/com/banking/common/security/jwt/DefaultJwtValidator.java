...
public class DefaultJwtValidator implements JwtValidator {
    ...
    public DefaultJwtValidator(String secret) {
        this.parser = Jwts.parserBuilder().setSigningKey(Keys.hmacShaKeyFor(secret.getBytes(secret.getBytes())))
                .build();
    }
    ...
    public JwtValidationResult validateToken (String jwt){
        try{
            ...
            return new JwtValidationResult(
                true, UUID.fromString(claims.getSubject()), Set<String>.of(claims.get("roles", String.class).split(","),
                null)
            );
        } catch (ExpiredJwtException e){
            log.warn("JWT expired at {}", e.getClaims().getExpiration());
            return new JwtValidationResult(false, null, null, ErrorCode.AUTH_TOKEN_INVALID);
